...
export class Matchers<T> {
  ...
  // Check to see if the value is an instance of the given consructor
  toBeInstanceOf(ctor: Constructable) {
    const isInstanceOf = this.actual instanceof ctor;
    if (this.inversed ? !isInstanceOf : isInstanceOf) {
      this.state.incPassed();
    } else {
      const actualCtor = (this.actual as any) as Constructable;
      const actualType = actualCtor.constructor.name
      const msg = `Fail - Expected value to be an instance of ${ctor.name}, got ${actualType}`;
      this.state.incFailed();
      throw new Error(msg);
    }
  }
