...
export class TestingState {
  ...
  public async runSuites() {
    for await (const [name, suite] of this.suites) {
      this.enterSuite(name);
      await suite.body();

      // Before All
      for await (const ba of suite.beforeAll) ba();

      // Each Test
      for await (const test of suite.tests) {

        // Before Each
        for await (const be of suite.beforeEach) be();
        ...
        // After Each
        for await (const ae of suite.afterEach) ae();
      }

      // After All
      for await (const aa of suite.afterAll) aa();
