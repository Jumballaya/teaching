...
export class BongoDBCollection<T> {
  ...
  public read<T>(query?: Partial<Indexable<T>>): Promise<BongoDBResponse<T>> {
    const obj = {
      entry: {
        read: {
          collection: this.name,
  ...
  public update<T>(updates: Partial<T>, query: Partial<Indexable<T>>): Promise<BongoDBResponse<T>> {
    const obj = {
      entry: {
        update: {
          collection: this.name,
  ...
  public delete<T>(query: Partial<Indexable<T>>): Promise<BongoDBResponse<T>> {
    const obj = {
      entry: {
        delete: {
          collection: this.name,
