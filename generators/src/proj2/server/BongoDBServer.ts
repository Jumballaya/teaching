import net from 'net';
import { Database } from '../../proj1/Database';
...
export class BongoDBServer {
  ...
  private handleEntryCommand(json: Command, socket: net.Socket, command: CommandType) {
    ...
    switch (command) {
      ...
      case 'entry:create': {
        ...
        if (cmd) {
          ...
          payload.forEach((p: unknown) => col.insert(p));
      ...
      case 'entry:read': {
        ...
        if (cmd) {
          ...
          socket.write(JSON.stringify({
            success: true,
            payload: collection.queryObject(query)
          }));
        }
        return;
      }
      ...
      case 'entry:update': {
        ...
        if (cmd) {
          ...
          collection.queryObject(query).map(v => collection.updateById(v._id, updates))
          socket.write(JSON.stringify({
            success: true,
            payload: collection.queryObject(query),
          }));
      ...
      case 'entry:delete': {
        ...
        if (cmd) {
          ...
          const out = collection.queryObject(query).map(v => collection.deleteById(v._id))
          socket.write(JSON.stringify({
            success: true,
            payload: out || [],
          }));
  ...
  private handleCollectionCommand(json: Command, socket: net.Socket, command: CommandType) {
